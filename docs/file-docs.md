...
### test/workers/run.js
Smoke test for Cloudflare Workers (`pnpm run test:workers`, needs `pnpm run build` first). It loads
the built `dist/index.js` into workerd, the Workers runtime, through Miniflare and fails if the
worker in `test/workers/worker.js` reports a failed check.

It is not a vitest project because `@cloudflare/vitest-pool-workers` only supports vitest 4. The
project stays on Miniflare 4 because Miniflare 5 is still an alpha with a different constructor API.
