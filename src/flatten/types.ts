import type { IsAny, Leaf, MergeVariants, Option, Prettify, ReplaceAll, UnionToIntersection } from '../shared/types.ts';
...
type Nested<V, K extends string, D extends unknown[], C extends Context> = V extends unknown
  ? Kind<V, C> extends 'unknown'
    ? { [_ in K | `${K}${C['delimiter']}${string}`]?: unknown }
...
type ObjectEntries<T, P extends string, D extends unknown[], C extends Context> = UnionToIntersection<
  { [K in keyof T]-?: K extends string | number ? Property<T[K], Join<P, Segment<K, C>, D, C>, IsOptional<T, K>, D, C> : {} }[keyof T]
>;
...
type FlatContainer<T, P extends string, D extends unknown[], C extends Context> = T extends readonly unknown[]
  ? number extends T['length']
    ? Property<T[number], Join<P, `${number}`, D, C>, false, D, C>
    : ObjectEntries<TupleEntries<T>, P, D, C>
  : ObjectEntries<T, P, D, C>;

type IsWide<C extends Context> = string extends C['delimiter'] ? true : number extends C['maxDepth'] ? true : boolean extends C['safe'] ? true : boolean extends C['keepEmpty'] ? true : boolean extends C['escape'] ? true : false;
